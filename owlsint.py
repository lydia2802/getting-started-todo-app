...
def start():
    ...
    if chose == "1":  # MENU 1
        ...
        if V == "1":
            ...
            print("")
            sleep(1)
            open_wa = input(f"{WHITE}TYPE 1 To Open Chat Target on WhatsApp or press ENTER to exit: ")
            if open_wa == "1":
                sleep(1)
                print(f"{WHITE}Opening WhatsApp link...")
                print(f"{GREEN}wa.me/{number}")
                sleep(0.5)
                print(f"{NORMAL}Done")
            elif open_wa == "":
                print("")
            ...
            number = input(f"{WHITE}Enter Number {NORMAL}({GREEN}Without +){WHITE} :{CRED2}➤ ")
            # Note: apilayer API key may need to be updated
            phe = requests.get("http://apilayer.net/api/validate?access_key=I2A2HXBuGbEJbufG8AxD2IGeFpqFyWFU&number=" + number)
            sleep(1)
            print()
            sleep(0.1)
        ...
        def instagram():
            ...
            try:
                ...
                f = il.Profile.from_username(x.context, uname)
                ...
                print("\033[32mPost count\033[0m :", f.mediacount)
                print("\033[32mProfile pic URL\033[0m :", f.profile_pic_url)
                print()

            except KeyboardInterrupt:
                print("\033[33mStopped!")
            except EOFError:
                print("\033[33mEOF Error!")

        instagram()

    elif chose in ("A", "a"):  # MENU ABOUT (fixed: was `chose == "A" or "a"` which always True)
        sleep(1)
        print(f"""{NORMAL}
...
""")
        ...
        print(f"{RED}Invalid option. Please choose 1, 2, 3, or A.")
